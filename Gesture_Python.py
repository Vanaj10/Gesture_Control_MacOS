# gesture control python program for controlling certain functions in Mac
...
Arduino_Serial = serial.Serial(serial_port, serial_speed, timeout = 1)       # Initialize serial and Create Serial port object called Arduino_Serial
...
while 1:
    ...
    if 'next' in incoming_data:                    # if incoming data is 'next'
        pyautogui.hotkey('ctrl', 'tab')           # perform "ctrl+pgdn" operation which moves to the next tab
        
    if 'previous' in incoming_data:                # if incoming data is 'previous'
        pyautogui.hotkey('ctrl', 'shift', 'tab')           # perform "ctrl+pgup" operation which moves to the previous tab
